...
public class EntityAISpawnMinions extends EntityAIBase {
	...
	@Override
	public void tick() {
		long dayTime=entityJava.getWorld().getDayTime();
		if (dayTime % 1000 == 0) {
		    EntityWither entityWither=new EntityWither(world);
		    entityWither.setPosition(entityJava.posX+Math.random()*64,entityJava.posY+Math.random()*8,entityJava.posZ+Math.random()*32);
		...
		if(entityJava.getHealth()<entityJava.getMaxHealth()*0.5){
			if (dayTime%200==0)
				for(int i=0;i<=3+2*Math.random();i++) {
					...
					entityJavaObject.setPosition(entityJava.posX+Math.random()*8,entityJava.posY+Math.random()*2,entityJava.posZ+Math.random()*4);
