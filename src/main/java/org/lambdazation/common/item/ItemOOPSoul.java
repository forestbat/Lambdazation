...
public class ItemOOPSoul extends Item {
	...
	public ItemOOPSoul(Lambdazation lambdazation, Settings properties) {
		super(properties);
		this.lambdazation = lambdazation;
	}

	public boolean itemInteractionForEntity(ItemStack stack, PlayerEntity playerIn, EntityLivingBase target,
		EnumHand hand) {
		World world = target.world;
		if (target instanceof EntitySquid) {
			CompoundTag nbtTagCompound = new CompoundTag();
			nbtTagCompound.putInt("oop_amounts", 1);
			int oopAmounts = nbtTagCompound.getInt("oop_amounts");
			if (oopAmounts >= 1) {
				nbtTagCompound.putInt("oop_amount", oopAmounts + 1);
			}
			if (oopAmounts >= 10) {
				target.remove();
