...
public final class BlockCharger extends Block {
	...
	public BlockCharger(Lambdazation lambdazation, Block.Settings properties) {
		super(properties);
	...
	public void onBlockRemoved(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
		if (state.getBlock() != newState.getBlock()) {
			...
			if (BlockEntity instanceof BlockEntityCharger) {
				dropStacks(state,worldIn, pos, (BlockEntityCharger) BlockEntity);
	...
	public boolean activate(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand hand,
		...
		//NetworkHooks.openGui(ServerPlayerEntity, blockEntityCharger, pos);
	...
	protected void appendProperties(StateFactory.Builder<Block, BlockState> builder) {
		FACING_PROPERTY_MAP.values().forEach(builder::with);
	}
}
